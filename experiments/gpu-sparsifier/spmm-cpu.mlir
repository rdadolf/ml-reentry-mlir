// CPU baseline for Day-0 Task 2.
//
// Hand-written sparse SpMM: C = A * B, A in CSR, B and C dense. Goes through
// `mlir-opt --sparsifier` (CPU pipeline) and is executed via mlir-runner.
// Output is captured for comparison against the GPU pipelines.
...
module {
  ...
  func.func @main() {
    ...
    // 8x8 matrix with ~1/3 sparsity: (i+j) when (i+j) % 3 == 0, else 0.
