// Test 5, baseline: SpMM with non-trivial sparse values (CSR).
//
// Y[i, k] = sum_j A[i, j] * X[j, k]
//
// 8x8 sparse A (attention-weighted adjacency-shape) times 8x4 dense X
// (per-node feature matrix), producing 8x4 dense Y. This is the GAT
// final-aggregation shape: weighted SpMM into a dense feature output.
...
module {
  ...
  func.func @weighted_spmm(%a: tensor<8x8xf32, #CSR>,
                           %x: tensor<8x4xf32>,
                           %y: tensor<8x4xf32>) -> tensor<8x4xf32> {
    %r = linalg.matmul
      ins(%a, %x : tensor<8x8xf32, #CSR>, tensor<8x4xf32>)
  ...
  func.func @main() {
    ...
    // 8x8 with values=(i+j)*0.5 at (i+j)%3==0 — distinct from Test 1.
    ...
    %r = call @weighted_spmm(%a, %x, %y)
      : (tensor<8x8xf32, #CSR>, tensor<8x4xf32>, tensor<8x4xf32>) -> tensor<8x4xf32>
