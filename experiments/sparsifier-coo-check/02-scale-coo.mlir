// Test 2, COO variant: sparse-in-place scale.
// Identical to 02-scale-csr.mlir except the encoding.
...
module {
  func.func @sparse_scale(%argx: tensor<8x8xf32, #COO>) -> tensor<8x8xf32, #COO> {
    %c = arith.constant 2.0 : f32
    %0 = linalg.generic #trait_scale
      outs(%argx: tensor<8x8xf32, #COO>) {
        ^bb(%x: f32):
          %1 = arith.mulf %x, %c : f32
          linalg.yield %1 : f32
    } -> tensor<8x8xf32, #COO>
    return %0 : tensor<8x8xf32, #COO>
  }
  ...
  func.func @main() {
    ...
    %s = sparse_tensor.convert %dense : tensor<8x8xf32> to tensor<8x8xf32, #COO>
    %r = call @sparse_scale(%s) : (tensor<8x8xf32, #COO>) -> tensor<8x8xf32, #COO>
