...
Reproducible:
[experiments/inductor-fusion/run.py](../inductor-fusion/run.py).
...
python experiments/pyg-baseline/run.py --disable-libs
# canonical compiled (torch.compile on top, same dispatch):
python experiments/pyg-baseline/run.py --disable-libs --compile
# legacy-compat check (libs installed, eager — mostly the same kernels):
python experiments/pyg-baseline/run.py
```
