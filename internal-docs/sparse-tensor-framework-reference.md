...
|---|---|---|
| **GCNConv** | `%arg1: !torch.vtensor<[N,N],f32,#sparse>` | **`torch.aten._sparse_mm(%arg1, %dense)`** — but only at `OutputType.RAW`, as an *unregistered* `torch.operator`. **It does not lower** (see correction below). |
| **SAGEConv** | Same `#sparse` signature | **`torch.aten._sparse_mm.reduce(%arg1, %arg0, "mean")`** (carries the mean-aggregation kind — *not* the plain `_sparse_mm` GCN emits) plus auxiliary `torch.aten.crow_indices(%arg1)` extraction for root-features handling. Because the reduce kind makes this not semantically a matmul, the GCN `_sparse_mm → aten.mm` despecialization does **not** apply; handling the reduce variant is separate work. |
...
- **CORRECTION (2026-05-29): GCN/SAGE are NOT end-to-end ready via this path.** The encoding survives *import*, but `torch.aten._sparse_mm` is an unregistered `torch.operator` with no torch-mlir conversion — `OutputType.{TORCH,LINALG_ON_TENSORS}` both fail to legalize it (`failed to legalize 'torch.operator'`). The earlier probe only checked `OutputType.RAW` (string presence at the import stage), not lowering. The sparsifier consumes `linalg` on a sparse-encoded tensor, **not** `_sparse_mm`. To bridge the gap, three options: (a) **canonicalize the `_sparse_mm` `torch.operator` to `torch.aten.mm`** — which *is* registered and lowers to `linalg.matmul` carrying the encoding. This is torch-mlir's *intended* sparse design (the encoding rides the normal dense op; there is no dedicated sparse-mm op), and `torch.spmm` already traces to `aten.mm` — cf. torch-mlir `test/python/fx_importer/sparsity/sparse_test.py::test_sparse_gcn`, which lowers a sparse GCN to linalg this way. (b) write the model with `torch.matmul` (emits the registered `aten.matmul`, which lowers the same way — verified). (c) rewrite the RAW torch-dialect IR in MLIR. **Current plan: emit RAW torch-dialect IR and canonicalize `_sparse_mm`→`aten.mm` in MLIR-land** — a tiny pass, not a new lowering. (Caveat: torch-mlir's in-tree sparse tests only exercise COO mm end-to-end, not CSR; treat CSR-SpMM-through-the-sparsifier as still-unverified. The torch-mlir sparse frontend itself stalled — last substantive work Dec 2024, ~all by Aart Bik.)
